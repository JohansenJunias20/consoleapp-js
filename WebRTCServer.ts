...
class WebRTCServer {
    ...
    private onJoinPeer(socketid: string) {
        ...
        var dcReliable = peer.createDataChannel("reliables", { ordered: false, maxRetransmits: 0 });
        dcReliable.onopen = (e) => {
    ...
    public broadcast_reliable(data: Buffer, socketid: string) {
        for (var key in this.peers) {
            ...
            if (peer.dcUnreliable.readyState == "open") {
                ...
                console.log("failed to send because state is: " + peer.dcUnreliable.readyState);
            }
        }
    }
